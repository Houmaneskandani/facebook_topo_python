...
def clientThread(conn):
	...
	if rcv_msg in userpass:
		...
		for n in range(len(requestlist)):
			if requestlist[n][2] == rcv_msg[0]:
				count = count + 1

		friendmsg = "You have " + str(count) + " unread friend request"
		conn.sendall(friendmsg)	 
		...
		while True:		
			...
			#=============================================
				...
				#	print (ms + " Time: " + currenttime + " \n")
					#print ms
					conn.sendall( ms + " recv at : " + str(currenttime) )
				except socket.error:
			...
			#============================================
				...
				friendid = conn.recv(1024)
				# check if id is already in buffer
				# if its not then add this user and the friend into request list
				# send back the confirmation
				for k in range(len(allusers)):
					print allusers[k][0]
					print friendid
					if allusers[k][0] == friendid :
						for i in range(len(requestlist)):
							if requestlist[i][0] == rcv_msg[0]: 
								conn.sendall('already sended the request')
							else:
								for f in range(len(connid)):
									if connid[f][0] == friendid:
										requestlist.append([rcv_msg[0],conn,friendid,connid[f][1]])
										conn.sendall('request has been send to ' + friendid ) 
									else:
										conn.sendall(' there is no request')
			...
			#=============================================
				...
				# print all the ids with this user 
				for i in range(len(requestlist)):
					if requestlist[i][2] == rcv_msg[0] :
						conn.sendall(' The user' + requestlist[i][0] + 'sends you request')
				...
				if n == str(1):
					for i in range(len(requestlist)):
						if requestlist[i][2] == rcv_msg[0]:
							friends.append([rcv_msg[0],conn,requestlist[i][0],requestlist[i][1]])
							requestlist.remove([requestlist[i][0],requestlist[i][1],rcv_msg[0],conn])
							conn.sendall(" accepted!")
				if n == str(2):
					for i in range(len(requestlist)):
						if requestlist[i][2] == rcv_msg[0]:
							requestlist.remove([requestlist[i][0],requestlist[i][1],rcv_msg[0],conn])
							conn.sendall(" rehected")
				else: 	
						print ' do nothing'	
			...
			#==============================================
				...
				for i in range(len(friends)):
					if friends[i][0] == rcv_msg[0]:
						k = k + 1
						conn.sendall( k + ". " + friends[i][2] )
